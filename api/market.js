...
function calcSupportResistance(candles, currentPrice) {
  ...
  // Cluster nearby pivots (within 1.5%)
  const clustered = [];
  pivots.forEach(p => {
  ...
  // Sort by strength, split above/below current price
...
export default async function handler(req, res) {
  ...
  try {
    // Core market data — all parallel
    const [quote, rsi, macd, ema50, ema200, earnings, ohlc, vixQ, spyQ, spyEma50, sectorQ, sectorEma50] = await Promise.all([
      td(`/quote?symbol=${sym}`),
    ...
    // Finnhub — IV proxy and analyst ratings
    const [finnhubQuote, recommendations] = await Promise.all([
      fh(`/quote?symbol=${sym}`),
      fh(`/stock/recommendation?symbol=${sym}`),
    ]);

    // Process OHLC for S/R and volume
    ...
    // Finnhub IV proxy — use high/low spread as IV estimate if options IV not available
    const fhPrice = finnhubQuote.c || currentPrice;
